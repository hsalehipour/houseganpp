...
import svgwrite
from utils import bb_to_img, bb_to_vec, bb_to_seg, mask_to_bb, remove_junctions, ID_COLOR, extract_rooms
...
for i, batch in enumerate(fp_loader):
    ...
    for k in range(opt.num_variations):
        ...
        with torch.no_grad():
            gen_mks, feats = generator(z, given_nds, given_eds)
            ...
            gen_masks_list.append(gen_mks)
            nodes_list.append(nds)
            edges_list.append(eds)

# save data
# feats_tensor = torch.cat(feats_tensor, 0)
# masks_tensor = torch.cat(masks_tensor, 0)
data = {'feats_list':feats_list, 'gen_masks_list':gen_masks_list, 'nodes_list':nodes_list, 'edges_list':edges_list}
np.save('./reconstruction_data.npy', data)
