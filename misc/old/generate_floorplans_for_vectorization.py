...
#         print('var num {}'.format(k))
        ...
        if k == 0:
            ...
            types, polys = extract_rooms(real_mks, real_nodes)
            
        # reconstruct
        types, polys = extract_rooms(gen_mks, real_nodes)
        conns = eds.detach().cpu().numpy()
        floors = [types, polys, conns]
        np.save('./raster/{}_{}.npy'.format(i, k), floors)