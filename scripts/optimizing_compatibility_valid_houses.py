...
def run_test(dict_states):
    ...
    all_states = [[all_types[l] for l in range(len(all_types)) if dict_states['var_{}'.format(l)] <= k] for k in range(10)]
    print([dict_states['var_{}'.format(k)] for k in range(len(all_types))])
    print(all_states)
    ...
    for i, sample in enumerate(fp_loader):
        ...
        avg_mistakes.append(1.0-compatibility_per_graph/100.0)    
...
for l in range(len(all_types)):
    space['var_{}'.format(l)] = hp.choice('var_{}'.format(l), tuple(range(10)))
