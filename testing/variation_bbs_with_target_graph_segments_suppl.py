...
numb_iters = 350000
exp_name = 'exp_with_shapes_attempt_2'
target_set = 'E'
phase='eval'
...
#         print('var num {}'.format(k))
        ...
        if k == 0:
            ...
            real_im_seg = draw_masks(real_mks, real_nodes)
...
#         final_images.append(fake_im_bb)
    ...
    if (n_rows+1)%128 == 0: 
        ...
        print(final_images)
        save_image(final_images, "./output/results_page_{}_{}.png".format(target_set, page_count), nrow=opt.num_variations+2, padding=2, range=(0, 1), pad_value=0.5, normalize=False)
        page_count += 1
        n_rows = 0
        final_images = []
