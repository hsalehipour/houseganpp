...
import cv2
import webcolors
def draw_masks(masks, ind_fixed_nodes, real_nodes):
...
#     img.paste(Image.alpha_composite(img, transp))
    ...
    for m, nd, ind_curr in zip(masks, real_nodes, range(masks.shape[0])):
        ...
        m = Image.fromarray(m)
        color = ID_COLOR[nd+1]
        if ind_curr in ind_fixed_nodes:
            color = 'black'
        r, g, b = webcolors.name_to_rgb(color)
        dr_reg.bitmap((0, 0), m.convert('L'), fill=(r, g, b, 128))
...
for i, batch in enumerate(fp_loader):
    ...
    # ind_fixed_nodes, bin_fixed_nodes = selectRandomNodes(nd_to_sample, opt.batch_size)
    ind_fixed_nodes = torch.tensor([])
...
#         print('var num {}'.format(k))
        ...
        with torch.no_grad():
            gen_mks = generator(z, None, given_masks_in, given_nds, given_eds)
        ...
        if k == 0:
            ...
            real_im_seg = draw_masks(real_mks, ind_fixed_nodes, real_nodes)
            final_images.append(real_im_seg)
        
            # Visualize given masks
            given_im_seg = draw_masks(given_masks.squeeze(1), ind_fixed_nodes, real_nodes)
            final_images.append(given_im_seg)

        # reconstruct
        fake_im_seg = draw_masks(gen_mks, ind_fixed_nodes, real_nodes)
    ...
    if (n_rows+1)%64 == 0: 
        ...
        save_image(final_images, "./output/results_GIVEN_MASK_PRIME_page_{}_{}.png".format(target_set, page_count), nrow=opt.num_variations+3, padding=2, range=(0, 1), pad_value=0.5, normalize=False)
        page_count += 1
        n_rows = 0
        final_images = []
